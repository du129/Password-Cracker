...
Key& Key::operator-=(const Key& other){
	...
	if(carry == true){
		//carry = 0;
		m_digit[i] = m_digit[i] - 1;
		if(m_digit[i] < 0){
			m_digit[i] = 31;
			//carry = true;
		}
		m_digit[i] = m_digit[i] - other.m_digit[i];
		if (m_digit[i] < 0) {
			m_digit[i] = 32 + m_digit[i];
			carry = true;
		} else {
			carry = false;
		}
		...
		//carry = 1;
		if(m_digit[i] < other.m_digit[i]){

			carry = true;
			m_digit[i] = m_digit[i] + 32 - other.m_digit[i];

		}else{
			carry = false;
			m_digit[i] -=  other.m_digit[i];
		}
	}
    }
    	return *this;
